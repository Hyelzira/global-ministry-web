...
const AdminEvents = () => {
  ...
  const openCreate = () => { setEditing(null); setForm(emptyForm()); setIsCancelledForm(false); setShowForm(true); };
  const openEdit   = (e: EventDto) => {
    ...
    setForm({
      ...
      acceptsDonations: e.acceptsDonations, donationLabel: e.donationLabel ?? '',
    });
    ...
    try {
      if (editing) {
        const updateDto: UpdateEventDto = {
          ...
          endDate: form.endDate, location: form.location, imageUrl: form.imageUrl,
          isCancelled: isCancelledForm, acceptsDonations: form.acceptsDonations, donationLabel: form.donationLabel,
        };
        const res = await eventApi.update(editing.id, updateDto);
        if (res.data.isSuccess) { toast.success('Updated'); setShowForm(false); fetchEvents(); }
      } else {
        const res = await eventApi.create(form);
        if (res.data.isSuccess) { toast.success('Created'); setShowForm(false); fetchEvents(); }
      }
            ...
            {events.map(e => (
              <div key={e.id} className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${t.row}`}>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => openEdit(e)} className={`p-2 rounded-lg transition-colors ${t.btnGhost}`} title="Edit">
                    <Pencil className={`w-4 h-4 ${t.subtext}`} />
                  </button>
                  <button onClick={() => setDeleteTarget(e)} className={`p-2 rounded-lg transition-colors ${t.btnGhost} hover:bg-red-500/20`} title="Delete">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className={`relative rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto border ${t.modal}`}>
            <div className={`flex items-center justify-between px-6 py-4 border-b ${t.border} sticky top-0`}
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              {/* Image upload — replaces the old imageUrl text field */}
              <ImageUpload
                value={form.imageUrl || ''}
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <div onClick={() => setForm(p => ({ ...p, acceptsDonations: !p.acceptsDonations }))}
                  className={`w-11 h-6 rounded-full transition-colors ${form.acceptsDonations ? 'bg-fuchsia-600' : t.toggle}`}>
              ...
              {form.acceptsDonations && (
                <input type="text" value={form.donationLabel} onChange={e => setForm(p => ({ ...p, donationLabel: e.target.value }))}
                  placeholder="Donation button label (optional)"
                  className={`w-full px-4 py-3 border rounded-xl text-sm outline-none ${t.modalInput}`} />
              )}
              {editing && (
                <label className="flex items-center gap-3 cursor-pointer">
                  <div onClick={() => setIsCancelledForm(v => !v)}
                    className={`w-11 h-6 rounded-full transition-colors ${isCancelledForm ? 'bg-red-600' : t.toggle}`}>
            ...
            <div className={`px-6 py-4 border-t ${t.border} flex gap-3 justify-end sticky bottom-0`}
              style={{ background: isDark ? '#161616' : 'white' }}>
